...
class Skeletons_Dataset(Dataset):
    """
    Allowed values:
    ...
    def __init__(self, csv_path, numberOfJoints=15, normalization='minmax', norm_precomputed_values=None,
                 target='cross', info='spatial', remove_undetected=False, transform=None):
        import pdb
        # pdb.set_trace()
        self.data = []
        self.n1 = 0
    ...
    def processData(self):
        ...
        skeletonData = self.loadedData['keypoints'].tolist() #taking 30 frames at a time concept of sliding window
        #print('total skeletons',len(skeletonData))
        ...
        if self.graphInfo == 'spatial':
            ...
            for iy, x in enumerate(x_values):
                ...
                """
                data_element = Data(x=x, y=y_values[iy], label=label_values[iy], edge_index=edge_index,
                                    num_nodes=x.shape[0], videoID=videoIDs[iy])
            ...
            for i in range(numFrames * 2 - 1, x_values.shape[0]):  # for num frames 3 (5, 239197)n
                ...
                if len(x_temp) == numFrames:
                    ...
                    data_element = Data(x_temporal=x_temp, y=y_value_i, label=label_i, edge_index=edge_index,
                                        num_nodes=x_values[currentFrame].shape[0], edge_weight=self.edge_weights,
                                        videoID=id_video_i)

                    self.data.append(data_element)

    def showSkeleton(self, videoNum=0, frameNum=0, textSize=14, showLegend=True, frameImage=None,
                     normalizedSkeletons=True,
        ...
        if normalizedSkeletons:
            ...
            neck_x = (skeleton[3][0] + skeleton[4][0]) / 2
            neck_y = (skeleton[3][1] + skeleton[4][1]) / 2  # adding Neck
            skeleton.append([neck_x, neck_y])
